- dashboard: voucher_dashboard
  ...
  - name: Voucher Redemptions
    ...
    explore: order_order
    type: looker_grid
    fields: [discount_voucher.code, discount_voucher.discount_value_type, discount_voucher.type,
      order_order.cnt_orders, order_order.cnt_unique_orders_new_customers, order_order.sum_discount_amt,
      order_order.avg_order_value_gross, discount_voucher.discount_value]
    filters:
      order_order.is_successful_order: 'yes'
      order_order.is_voucher_order: 'Yes'
    sorts: [order_order.cnt_orders desc]
    limit: 5000
    total: true
    dynamic_fields: [{table_calculation: acquisition_share, label: "% Acquisition\
          \ Share", expression: " ${order_order.cnt_unique_orders_new_customers} /${order_order.cnt_orders}",
    ...
    series_column_widths:
      order_order.avg_order_value_gross: 127
      discount_voucher.discount_value: 87
    column_order: ["$$$_row_numbers_$$$", discount_voucher.code, discount_voucher.discount_value_type,
      discount_voucher.discount_value, discount_voucher.type, order_order.cnt_orders,
      order_order.cnt_unique_orders_new_customers, order_order.avg_order_value_gross,
      order_order.sum_discount_amt, acquisition_share]
    hidden_fields: []
    y_axes: []
    listen:
      Is Internal Order (Yes / No): order_order.is_internal_order
      Hub Name: hubs.hub_name
      Country: hubs.country
      Voucher Code: discount_voucher.code
      Voucher Type: discount_voucher.type
      Voucher Use Case: discount_voucher.use_case
      Order Date (Only applies to the top Look): order_order.created_date
    row: 0
    col: 0
    width: 24
    height: 11
  - title: Voucher Retention
    ...
    type: looker_grid
    fields: [voucher_retention.code, voucher_retention.cnt_base_7, voucher_retention.cnt_base_14,
      voucher_retention.cnt_base_30, voucher_retention.cnt_7_day_retention, voucher_retention.cnt_14_day_retention,
    ...
    listen:
      ...
      Voucher Code: voucher_retention.code
    row: 14
    col: 0
    width: 18
    height: 13
  ...
  - name: Voucher Use Case
    ...
    explore: order_order
    listens_to_filters: []
    field: discount_voucher.use_case
