view: botton_10_hubs {
  derived_table: {
    sql: with hubs_kpis as (
          ...
          , (((sum(number_of_promoter)/nullif(sum(number_of_nps_responses), 0))* 100) - ((sum(number_of_detractors)/nullif(sum(number_of_nps_responses), 0))* 100)) as nps_score
      ...
      ranked as (select *
          ...
          , row_number() OVER (PARTITION BY order_date ORDER BY nps_score asc) as nps_rank
      from hubs_kpis
      ...
      select country_iso
          , hub_code
          , order_date
          , if(delivery_rank<=10, 1, 0) as is_bottom_10_delivery
          , if(issues_rank<=10, 1, 0) as is_bottom_10_issues
  ...
  set: detail {
    fields: [hub_code, order_date, is_bottom_10_delivery, is_bottom_10_issues, is_bottom_10_nps]
  }
}
