        ...
        sql: with order_ranks as
                (
                        SELECT
                                ...
                                FROM `flinkdataprod.saleor_prod_global.order_order` order_order
                      ...
                      (
                              ...
                              from (
                                            ...
                                            from (
                                                            ...
                                                            CASE WHEN JSON_EXTRACT_SCALAR(b.metadata, '$.warehouse') IN ('hamburgoellkersallee', 'hamburgoelkersallee') THEN 'de_ham_alto'
                                                                    WHEN JSON_EXTRACT_SCALAR(b.metadata, '$.warehouse') = 'münchenleopoldstraße' THEN 'de_muc_schw'
                                                                    ELSE JSON_EXTRACT_SCALAR(b.metadata, '$.warehouse') end as warehouse,
                                                            user_order_rank,
                                                            order_id,
                                                            from (select order_id, country_iso, CASE WHEN LENGTH(product_sku)=7 THEN CONCAT('1', product_sku) ELSE product_sku END as product_sku from `flinkbackend.saleor_db_global.order_orderline`) a
                                                            left join `flinkbackend.saleor_db_global.order_order` b
                                                            on a.order_id = b.id and a.country_iso = b.country_iso
                                                            left join `flinkbackend.saleor_db_global.product_productvariant` pvariant
                                                            on a.product_sku=pvariant.sku and a.country_iso=pvariant.country_iso
                                                            left join `flinkbackend.saleor_db_global.product_product` pproduct
                                                            on pvariant.id=pproduct.id and pvariant.country_iso = pproduct.country_iso
                                                            left join `flinkbackend.saleor_db_global.product_category` pcategory
                                                            on pproduct.category_id=pcategory.id and pproduct.country_iso = pcategory.country_iso
                                                            left join `flinkbackend.saleor_db_global.product_category` pcategory_parent
                                                            ...
                                                            where b.status in ('fulfilled', 'partially fulfilled') and b.created > '20210225'
                              ...
                              (
                                    select
                                            ...
                                            (
                                                    ...
                                                    CASE WHEN JSON_EXTRACT_SCALAR(c.metadata, '$.warehouse') IN ('hamburgoellkersallee', 'hamburgoelkersallee') THEN 'de_ham_alto'
                                                    WHEN JSON_EXTRACT_SCALAR(c.metadata, '$.warehouse') = 'münchenleopoldstraße' THEN 'de_muc_schw'
                                                    ELSE JSON_EXTRACT_SCALAR(c.metadata, '$.warehouse') end as warehouse
                                                    from (select order_id, country_iso, CASE WHEN LENGTH(product_sku)=7 THEN CONCAT('1', product_sku) ELSE product_sku END as product_sku from `flinkbackend.saleor_db_global.order_orderline`) a,
                                                    (select order_id, country_iso, CASE WHEN LENGTH(product_sku)=7 THEN CONCAT('1', product_sku) ELSE product_sku END as product_sku from `flinkbackend.saleor_db_global.order_orderline`) b
                                                    left join `flinkbackend.saleor_db_global.order_order` c
                                                    ...
                                                    left join `flinkbackend.saleor_db_global.product_category` pcategory_parent
                                                    on pcategory.parent_id=pcategory_parent.id and pcategory.country_iso = pcategory_parent.country_iso
                                                    left join `flinkbackend.saleor_db_global.product_productvariant` pvariant_2
                                                    on b.product_sku=pvariant_2.sku and b.country_iso = pvariant_2.country_iso
                                                    left join `flinkbackend.saleor_db_global.product_product` pproduct_2
                                                    on pvariant_2.id=pproduct_2.id and pvariant_2.country_iso = pproduct_2.country_iso
                                                    left join `flinkbackend.saleor_db_global.product_category` pcategory_2
                                                    on pproduct_2.category_id=pcategory_2.id and pproduct_2.country_iso = pcategory_2.country_iso
                                                    left join `flinkbackend.saleor_db_global.product_category` pcategory_parent_2
                                                    ...
                                                    pcategory_parent.id < pcategory_parent_2.id and
                                                    c.status in ('fulfilled', 'partially fulfilled') and c.created > '20210225'
                      ...
                      (
                              ...
                              from (
                                            ...
                                            (
                                                    ...
                                                    CASE WHEN JSON_EXTRACT_SCALAR(d.metadata, '$.warehouse') IN ('hamburgoellkersallee', 'hamburgoelkersallee') THEN 'de_ham_alto'
                                                    WHEN JSON_EXTRACT_SCALAR(d.metadata, '$.warehouse') = 'münchenleopoldstraße' THEN 'de_muc_schw'
                                                    ELSE JSON_EXTRACT_SCALAR(d.metadata, '$.warehouse') end as warehouse
                                                    from (select order_id, country_iso, CASE WHEN LENGTH(product_sku)=7 THEN CONCAT('1', product_sku) ELSE product_sku END as product_sku from `flinkbackend.saleor_db_global.order_orderline`) a,
                                                    (select order_id, country_iso, CASE WHEN LENGTH(product_sku)=7 THEN CONCAT('1', product_sku) ELSE product_sku END as product_sku from `flinkbackend.saleor_db_global.order_orderline`) b,
                                                    (select order_id, country_iso, CASE WHEN LENGTH(product_sku)=7 THEN CONCAT('1', product_sku) ELSE product_sku END as product_sku from `flinkbackend.saleor_db_global.order_orderline`) c
                                                    left join `flinkbackend.saleor_db_global.order_order` d
                                                    ...
                                                    left join `flinkbackend.saleor_db_global.product_category` pcategory_parent_2
                                                    on pcategory_2.parent_id=pcategory_parent_2.id and pcategory_2.country_iso = pcategory_parent_2.country_iso
                                                    left join `flinkbackend.saleor_db_global.product_productvariant` pvariant_3
                                                    on c.product_sku=pvariant_3.sku and c.country_iso = pvariant_3.country_iso
                                                    left join `flinkbackend.saleor_db_global.product_product` pproduct_3
                                                    on pvariant_3.id=pproduct_3.id and pvariant_3.country_iso = pproduct_3.country_iso
                                                    left join `flinkbackend.saleor_db_global.product_category` pcategory_3
                                                    on pproduct_3.category_id=pcategory_3.id and pproduct_3.country_iso = pcategory_3.country_iso
                                                    left join `flinkbackend.saleor_db_global.product_category` pcategory_parent_3
                                                    ...
                                                    pcategory_parent.id < pcategory_parent_3.id and
                                                    pcategory_parent_2.id < pcategory_parent_3.id and
                                                    d.status in ('fulfilled', 'partially fulfilled') and d.created > '20210225'
