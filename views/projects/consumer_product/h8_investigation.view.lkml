view: h8_investgation {
  derived_table: {
    ...
    sql: WITH
        ...
        SELECT
          tracks.anonymous_id,
          tracks.context_app_build, tracks.context_app_version,
          # CAST(NULL AS BOOL) AS context_device_ad_tracking_enabled,
          tracks.context_device_id, tracks.context_device_manufacturer, tracks.context_device_model, tracks.context_device_name, tracks.context_device_type, tracks.context_ip, tracks.context_library_name, tracks.context_library_version, tracks.context_locale,
          # CAST(NULL AS BOOL) AS context_network_bluetooth, tracks.context_network_carrier, tracks.context_network_cellular, tracks.context_network_wifi,
          tracks.context_os_name, tracks.context_os_version, tracks.context_protocols_source_id, tracks.context_timezone,
          # CAST(NULL AS STRING) AS context_traits_anonymous_id, CAST(NULL AS STRING) AS context_user_agent,
          tracks.event, tracks.event_text, tracks.id, tracks.original_timestamp, tracks.timestamp,
      ...
      ON
        ...
        AND (location_table.timestamp < tracking_sessions.next_session_start_at
          OR tracking_sessions.next_session_start_at IS NULL)
      WHERE
        location_table.event='voucher_redemption_attempted'
      GROUP BY
        1, 2 ),

        api_voucher_apply_failed AS (
      SELECT
        tracking_sessions.anonymous_id,
        tracking_sessions.session_id,
        COUNT(*) AS event_count
      ...
      ON
        ...
        AND (location_table.timestamp < tracking_sessions.next_session_start_at
          OR tracking_sessions.next_session_start_at IS NULL)
      WHERE
        location_table.event='api_voucher_apply_failed'
      GROUP BY
        1, 2 ),

        api_voucher_apply_succeeded AS (
      SELECT
        tracking_sessions.anonymous_id,
        tracking_sessions.session_id,
        COUNT(*) AS event_count
      ...
      ON
        ...
        AND (location_table.timestamp < tracking_sessions.next_session_start_at
          OR tracking_sessions.next_session_start_at IS NULL)
      WHERE
        location_table.event='api_voucher_apply_succeeded'
      GROUP BY
        1, 2 ),

        api_delivery_area_failed AS (
      SELECT
        tracking_sessions.anonymous_id,
        tracking_sessions.session_id,
        COUNT(*) AS event_count
      ...
      ON
        ...
        AND (location_table.timestamp < tracking_sessions.next_session_start_at
          OR tracking_sessions.next_session_start_at IS NULL)
      WHERE
        location_table.event='api_delivery_area_failed'
      GROUP BY
        1, 2 ),

        api_delivery_area_succeeded AS (
      SELECT
        tracking_sessions.anonymous_id,
        tracking_sessions.session_id,
        COUNT(*) AS event_count
      ...
      ON
        ...
        AND (location_table.timestamp < tracking_sessions.next_session_start_at
          OR tracking_sessions.next_session_start_at IS NULL)
      WHERE
        location_table.event='api_delivery_area_succeeded'
      GROUP BY
        1, 2 ),
      ...
      ON
        ...
        AND (location_table.timestamp < tracking_sessions.next_session_start_at
          OR tracking_sessions.next_session_start_at IS NULL)
      WHERE
        location_table.event='waitlist_signup_selected'
      GROUP BY
        1, 2 )
      ...
      SELECT
        tracking_sessions.anonymous_id,
        tracking_sessions.session_id,
        datetime(tracking_sessions.session_start_at,'Europe/Berlin') AS session_start_at,
        datetime(tracking_sessions.next_session_start_at,'Europe/Berlin') AS next_session_start_at,
        ...
        voucher_redemption_attempted.event_count  AS voucher_redemption_attempted,
        api_voucher_apply_failed.event_count AS api_voucher_apply_failed,
        api_voucher_apply_succeeded.event_count AS api_voucher_apply_succeeded,
        api_delivery_area_failed.event_count AS api_delivery_area_failed,
        # api_delivery_area_succeeded.event_count AS api_delivery_area_succeeded,
        waitlist_signup_selected.event_count as waitlist_signup_selected
      ...
      LEFT JOIN voucher_redemption_attempted  ON tracking_sessions.session_id = voucher_redemption_attempted.session_id
      LEFT JOIN api_voucher_apply_failed ON tracking_sessions.session_id = api_voucher_apply_failed.session_id
      LEFT JOIN api_voucher_apply_succeeded ON tracking_sessions.session_id = api_voucher_apply_succeeded.session_id
      LEFT JOIN api_delivery_area_failed ON tracking_sessions.session_id = api_delivery_area_failed.session_id
      # LEFT JOIN api_delivery_area_succeeded ON tracking_sessions.session_id = api_delivery_area_succeeded.session_id
      LEFT JOIN waitlist_signup_selected ON tracking_sessions.session_id = waitlist_signup_selected.session_id
  ...
  dimension: voucher_apply_failed {
    type: number
    sql: ${TABLE}.api_voucher_apply_failed ;;
  }

  dimension: voucher_apply_succeeded {
    type: number
    sql: ${TABLE}.api_voucher_apply_succeeded ;;
  }

  dimension: delivery_area_failed {
    type: number
    sql: ${TABLE}.api_delivery_area_failed ;;
  }

  dimension: delivery_area_succeeded {
    type: number
    sql: ${TABLE}.api_delivery_area_succeeded ;;
  }
...
#   }
  ...
  measure: cnt_voucher_redemption_attempted {
    label: "Voucher Redemption Attempted count"
    description: "Number of sessions in which at least one Voucher Redemption Attempt event happened"
    type: count
    filters: [voucher_redemption_attempted: "NOT NULL"]
  }

  measure: cnt_voucher_apply_failed {
    label: "Voucher Apply Failed count"
    description: "Number of sessions in which at least one Voucher Applied Failed event happened"
    type: count
    filters: [voucher_apply_failed: "NOT NULL"]
  }

  measure: cnt_voucher_apply_succeeded {
    label: "Voucher Apply Successed count"
    description: "Number of sessions in which at least one Voucher Apply Success event happened"
    type: count
    filters: [voucher_apply_succeeded: "NOT NULL"]
  }

  measure: cnt_delivery_area_failed {
    label: "Delivery Area Failed count"
    description: "Number of sessions in which at least one Delivery Area Failed event happened"
    type: count
    filters: [delivery_area_failed: "NOT NULL"]
  }

  # measure: cnt_delivery_area_succeeded{
  #   label: "Delivery Area Succeeded count"
  #   description: "Number of sessions in which at least one Delivery Area Succeeded event happened"
  #   type: count
  #   filters: [delivery_area_succeeded: "NOT NULL"]
  # }

  measure: cnt_waitlist_signup_selected {
    label: "Wailist Signup Selected count"
    description: "Number of sessions in which at least one Waitlist Signup Selected event happened"
    type: count
    filters: [waitlist_signup_selected: "NOT NULL"]
  }
  ...
  measure: sum_vouchers_redemption_attempted {
    label: "Voucher Redemption Attempted sum of events"
    type: sum
    sql: ${voucher_redemption_attempted} ;;
  }

  measure: sum_vouchers_failed {
    label: "Voucher Apply Failed sum of events"
    type: sum
    sql: ${voucher_apply_failed} ;;
  }

  measure: sum_vouchers_succeeded{
    label: "Voucher Apply Successed sum of events"
    type: sum
    sql: ${voucher_apply_succeeded} ;;
  }

  measure: sum_area_failed {
    label: "Delivery Area Failed sum of events"
    type: sum
    sql: ${delivery_area_failed} ;;
  }

  measure: sum_area_succeeded {
    label: "Delivery Area Succeeded sum of events"
    type: sum
    sql: ${delivery_area_succeeded} ;;
  }

  measure: sum_waitlist_signup_clicked {
    label: "Wailist Signup Selected sum of events"
    type: sum
    sql: ${waitlist_signup_selected} ;;
  }
  ...
  measure: voucher_redemption_attempted_rate {
    type: number
    description: "Number of sessions in which an Voucher Redemotion Attempted event happened, compared to the total number of Session Started"
    value_format_name: percent_1
    sql: ${cnt_voucher_redemption_attempted}/NULLIF(${count},0) ;;
  }

  measure: voucher_apply_failed_rate {
    type: number
    description: "Number of sessions in which an Voucher Apply Failed event happened, compared to the total number of Session Started"
    value_format_name: percent_1
    sql: ${cnt_voucher_apply_failed}/NULLIF(${count},0) ;;
  }

  measure: voucher_apply_succeeded_rate {
    type: number
    description: "Number of sessions in which an Voucher Apply Succeeded event happened, compared to the total number of Session Started"
    value_format_name: percent_1
    sql: ${cnt_voucher_apply_succeeded}/NULLIF(${count},0) ;;
  }

  measure: delivery_area_failed_rate {
    type: number
    description: "Number of sessions in which an Delivery Area Failed event happened, compared to the total number of Session Started"
    value_format_name: percent_1
    sql: ${cnt_delivery_area_failed}/NULLIF(${count},0) ;;
  }

  # measure: delivery_area_succeeded_rate {
  #   type: number
  #   description: "Number of sessions in which an Delivery Area Succeede event happened, compared to the total number of Session Started"
  #   value_format_name: percent_1
  #   sql: ${cnt_delivery_area_succeeded/NULLIF(${count},0) ;;
  # }

  measure: waitlist_signup_selected_rate {
    type: number
    description: "Number of sessions in which an Waitlist Signup Selected event happened, compared to the total number of Session Started"
    value_format_name: percent_1
    sql: ${cnt_waitlist_signup_selected}/NULLIF(${count},0) ;;
  }
  ...
  set: detail {
    fields: [
      ...
      voucher_apply_succeeded,
      delivery_area_failed,
      delivery_area_succeeded,
