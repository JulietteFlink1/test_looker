include: "/views/bigquery_tables/curated_layer/orders_cleaned_v2.view"
...
include: "/explores/base_explores/orders_cl_cleaned_v2.explore"

#explore: nps_after_order_cl { hidden:yes }

explore: orders_cl_cleaned_v2 {
  from: orders_cleaned_v2
  label: "Orders"
  view_label: "* Orders *"
  group_label: "01) Performance"
  description: "General Business Performance - Orders, Revenue, etc."
  view_name: orders_cl_cleaned_v2  # needs to be set in order that the base_explore can be extended and referenced properly
  hidden: no

  always_filter: {
    filters:  [
      orders_cl_cleaned_v2.is_internal_order: "no",
  ...
  access_filter: {
    field: orders_cl_cleaned_v2.country_iso
  ...
  join: hubs {
    from: hubs_ct
    view_label: "* Hubs *"
    sql_on: lower(${orders_cl_cleaned_v2.hub_code}) = ${hubs.hub_code} ;;
  ...
  join: shyftplan_riders_pickers_hours {
    ...
    view_label: "* Shifts *"
    sql_on: ${orders_cl_cleaned_v2.created_date} = ${shyftplan_riders_pickers_hours.date} and
  ...
  join: nps_after_order {
    ...
    view_label: "* NPS CL*"
    sql_on: ${orders_cl_cleaned_v2.country_iso}   = ${nps_after_order.country_iso} AND
      ${orders_cl_cleaned_v2.order_number}  =       ${nps_after_order.order_number} ;;
