...
view: employee_level_kpis {
  ...
  dimension: utr_dimension {
    type:  number
    hidden: yes
    sql: ${TABLE}.number_of_delivered_orders/nullif(${TABLE}.number_of_worked_hours,0);;
  }

  dimension: utr_dimension_tiered {
    label: "UTR Tier"
    type: tier
    tiers: [0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4, 1.6, 1.8, 2, 2.2, 2.4, 2.6, 2.8, 3, 3.2, 3.4, 3.6, 3.8, 4, 4.2, 4.4, 4.6, 4.8, 5]
    style: relational
    sql: ${utr_dimension} ;;
  }
  ...
  measure: sum_picking_time_minutes {
    ...
    description: "Sum of the time spent doing picking activities based on the Hub One data."
    sql: ${TABLE}.number_of_picking_time_minutes ;;
    value_format_name: decimal_1
  }
  ...
  measure: avg_picking_time_order {
    ...
    description: "Average time needed to pick items per order."
